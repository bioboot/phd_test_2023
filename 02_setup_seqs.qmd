---
title: "Setup Mutation Sequence 02 TP53"
author: "Barry Grant"
date: 2023-05-08
format:
  html:
...
Usage example, generate 6 mutations per student drawn from the kinase domain positions 457 to 717:

```{r}

mutant_sites <- random_mutations(students$PID, bounds=470:673)
row.names(mutant_sites) <- students$PID
...
wt <- get.seq(id=id)
```
The P15056 SP ID corresponds to BRAF a.k.a. Serine/threonine-protein kinase B-raf.
...
mutant_sites <- random_mutations(students$PID, bounds=100:260)
row.names(mutant_sites) <- students$PID
